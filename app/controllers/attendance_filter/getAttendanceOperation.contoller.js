...
const getAttendanceOperation = (async(request, response) => {
	...
	try{
		...
		query = `
			...
			where att = 'p' and MONTH(att_date) = ? and YEAR(att_date) = ?
			group by attendance.sid limit ?, 10;
		`;
		
		let result = await db.executeQuery(query, [month, year, page.currentPageno*10]);
